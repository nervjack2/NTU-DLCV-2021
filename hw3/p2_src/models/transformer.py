...
class Transformer(nn.Module):
    ...
    def forward(self, src, mask, pos_embed, tgt, tgt_mask):
        ...
        hs, att_w = self.decoder(tgt, memory, memory_key_padding_mask=mask, tgt_key_padding_mask=tgt_mask,
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, tgt, memory,
        ...
        att_weights = []
        for layer in self.layers:
            output, att_w = layer(output, memory, tgt_mask=tgt_mask,
        ...
        if self.return_intermediate:
            return torch.stack(intermediate), att_w
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward_post(self, tgt, memory,
        ...
        tgt = self.norm1(tgt)
        tgt2, att_w = self.multihead_attn(query=self.with_pos_embed(tgt, query_pos),
    ...
    def forward_pre(self, tgt, memory,
        ...
        tgt = tgt + self.dropout1(tgt2)
        tgt2 = self.norm2(tgt)
        tgt2, att_w = self.multihead_attn(query=self.with_pos_embed(tgt2, query_pos),
                                   key=self.with_pos_embed(memory, pos),
                                   value=memory, attn_mask=memory_mask,
                                   key_padding_mask=memory_key_padding_mask)
